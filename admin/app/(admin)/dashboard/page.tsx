  ...
  return (
    <div>
        ...
        <div className="flex gap-2">
          {DATE_RANGES.map(r => (
            <button key={r.days} onClick={() => setDateRange(r.days)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${dateRange === r.days ? "bg-indigo-600 text-white" : "bg-white text-slate-600 border border-slate-300"}`}>
              {r.label}
            </button>
          ))}
        </div>
      ...
      <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm mb-6">
        <h2 className="text-lg font-semibold text-slate-900 mb-4">Transfer Volume</h2>
        <div className="h-64">
          {volumeData.length > 0 ? (
