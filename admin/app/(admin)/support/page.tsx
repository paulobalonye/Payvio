...
export default function SupportPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-6">Support Tools</h1>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6 mb-6">
        ...
        <div className="flex gap-3 mb-4">
          <input value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleLookup()}
            ...
            className="flex-1 border border-slate-300 dark:border-slate-600 rounded-lg px-4 py-2 text-sm outline-none" />
            ...
            {results.users?.length > 0 && (
              <div>
                ...
                {results.users.map((u: any) => (
                  <div key={u.id} className="flex items-center justify-between py-2 border-b border-slate-100 dark:border-slate-700">
                    <div>
            ...
            {results.transfers?.length > 0 && (
              <div>
                ...
                {results.transfers.map((t: any) => (
                  <div key={t.id} className="py-2 border-b border-slate-100 dark:border-slate-700 text-sm">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
        ...
        <input value={notifUserId} onChange={(e) => setNotifUserId(e.target.value)} placeholder="User ID"
          className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-4 py-2 text-sm mb-3 outline-none" />
        <select value={notifChannel} onChange={(e) => setNotifChannel(e.target.value)} className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-4 py-2 text-sm mb-3">
        ...
        <textarea value={notifMessage} onChange={(e) => setNotifMessage(e.target.value)} placeholder="Message..."
          rows={3} className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-4 py-3 text-sm mb-3 outline-none" />
