          ...
          <tbody>
            {configs.map(c => (
              <tr key={c.id} className="border-b border-slate-100">
                ...
                <td className="px-6 py-4">{c.overrideRate ? <span className="px-2 py-1 bg-orange-100 text-orange-700 rounded-full text-xs">Active</span> : "—"}</td>
                <td className="px-6 py-4">
                  {editing === c.corridor ? (
            ...
            {configs.length === 0 && <tr><td colSpan={5} className="px-6 py-12 text-center text-slate-400">No rate configs yet. They'll appear when corridors are configured.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
